...
int isDirExistProductVersion(char userName[]){
    char directoryPath[50];
    strcpy(directoryPath,"ProductDataBase\\");
    strcat(directoryPath,userName);
    DIR *d;
    int result=1;
    d = opendir(directoryPath);
...
int createDirProductEdition(char productName[]){
    char path[50];
    strcpy(path,"ProductDataBase\\");
    strcat(path,productName);
    int result = mkdir(path);
    return result;
}
...
void readProductData(char *filePath,char *userData){
FILE *fptr=fopen(filePath,"r");
fgets(userData,200,fptr);
fclose(fptr);
}
